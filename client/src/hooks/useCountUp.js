...
/**
 * Анимирует число от 0 до `end` за `duration` мс с easing-out cubic.
 *
 * Анимация запускается только когда `start === true`. До этого момента
 * хук возвращает 0. Это позволяет триггерить отсчёт, например, при
 * попадании элемента в viewport.
 *
